    ...
    <Styles.ProfileContainer>
      {
        isLoading ? (
          ...
          <Styles.Image src={`${baseUrlApi}/${profile?.image}`} alt='Profile photo' />
        )
      }
      <Styles.Content>
        <Styles.Name className={isLoading ? 'skeleton' : ''}>{profile?.name}</Styles.Name>
        <Styles.Role className={isLoading ? 'skeleton' : ''}>{profile?.role}</Styles.Role>
      </Styles.Content>
